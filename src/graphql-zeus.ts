...
export type ValueTypes = {
  ...
  ["Mutation"]: AliasType<{
    addLanguage?: [
      { language: ValueTypes["AddLanguageInput"] },
      ValueTypes["Language"]
    ];
    addPhrase?: [{ phrase: ValueTypes["PhraseInput"] }, ValueTypes["Phrase"]];
    addTemplate?: [
      { template: ValueTypes["MachineTemplateInput"] },
      ValueTypes["MachineTemplate"]
    ];
    addTranslation?: [
      { translation: ValueTypes["createTranslation"] },
    ...
    upsertTemplateComponent?: [
      { component: ValueTypes["TemplateComponentInput"] },
      ValueTypes["TemplateComponent"]
    ];
    __typename?: true;
  }>;
  ...
  ["Query"]: AliasType<{
    GenerateMachineDocument?: [
      { language: string; machine: string; version?: string },
      ValueTypes["MachineDocument"]
    ];
    autocomplete?: [{ phrase: ValueTypes["PhraseInput"] }, true];
    findTemplates?: [
      { filter?: ValueTypes["FindTemplatesInput"] },
      ValueTypes["MachineTemplate"]
    ];
    generatePDF?: [
      { machine: string; version?: string; language: string },
      true
    ];
...
export type PartialObjects = {
  ...
  ["Feature"]:
    | PartialObjects["TextBlock"]
    | PartialObjects["Image"]
    | PartialObjects["TableBlock"]
    | PartialObjects["Columns"]
    | PartialObjects["Stack"]
    | PartialObjects["ListBlock"]
    | PartialObjects["TimeStamp"];
  ["FeatureInput"]: {
    styleJson?: string;
    colspan?: number;
    width?: number;
  ...
  ["Margin"]: {
    ...
    | PartialObjects["Column"]
    | PartialObjects["Columns"]
    | PartialObjects["Features"]
    | PartialObjects["Image"]
    | PartialObjects["ListBlock"]
    | PartialObjects["MachineDocument"]
    | PartialObjects["Row"]
    | PartialObjects["Stack"]
    | PartialObjects["TableBlock"]
    | PartialObjects["TextBlock"]
...
> = SRC extends {
  ...
  __resolve: infer IMPLEMENTORS extends Record<string, { __typename: any }>;
}
  ? ObjectToUnion<
      Omit<
        {
          [Key in keyof Omit<DST, keyof INTERFACE | '__typename'>]: Key extends keyof IMPLEMENTORS
            ? MapType<IMPLEMENTORS[Key], DST[Key]> &
                Omit<
                  {
                    [K in keyof Omit<DST, keyof IMPLEMENTORS | '__typename'>]: K extends keyof INTERFACE
                      ? LastMapTypeSRCResolver<INTERFACE[K], DST[K]>
                      : never;
                  },
                  keyof IMPLEMENTORS
                > &
...
const queryConstruct =
  (t: "Query" | "Mutation" | "Subscription") => (o: Record<any, any>) =>
